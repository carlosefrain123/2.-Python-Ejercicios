...
import stock_pasillos

class TestGestionStock(unittest.TestCase):
    ...
    def test_structure_try(self):
        source_code = inspect.getsource(stock_pasillos)
    ...
    def test_print_inicial(self, mock_print):
        reload(stock_pasillos)
        mock_print.assert_any_call('**** Calcular El Stock Total Por Pasillo Y Categoría ****')

    # *** QUINTO ESCENARIO => Datos inválidos en pasillos/categorías ***
    ...
    def test_datos_invalidos_principales(self, mock_input):
        ...
        self.assertIn('Datos De Pasillos Y/O Categorias No Son Válidos', output, '❌ No valida valores <= 0')

    # *** SEXTO ESCENARIO => Stock negativo en categoría ***
    @patch('builtins.input', side_effect=['2', '2', '10', '-5'])
    def test_stock_negativo(self, mock_input):
        reload(stock_pasillos)
        
        output = self.stdout_capture.getvalue()
        
        self.assertIn('Dato En Stock No Es Válido', output, '❌ No detecta stock negativo')

    # *** SÉPTIMO ESCENARIO => Flujo válido 2 pasillos x 3 categorías ***
    @patch('builtins.input', side_effect=['2', '3', '10', '20', '30', '5', '15', '25'])
    def test_flujo_valido(self, mock_input):
        reload(stock_pasillos)
        
        output = self.stdout_capture.getvalue()
        
        self.assertIn('Total Productos En Stock Del Pasillo #1: 60', output, '❌ Cálculo pasillo 1 incorrecto')
        self.assertIn('Total Productos En Stock Del Pasillo #2: 45', output, '❌ Cálculo pasillo 2 incorrecto')
    ...
    def test_caso_minimo(self, mock_input):
        ...
        self.assertIn('Total Productos En Stock Del Pasillo #1: 0', output, '❌ Caso 1x1 falló')
    ...
    def test_input_messages(self, mock_input):
        ...
        expected_prompts = [
            'Ingrese El Número De Pasillos: ',
            'Ingrese La Cantidad De Categorías Por Pasillo: ',
            'Productos En Stock De La Categoría #1: '
        ]
