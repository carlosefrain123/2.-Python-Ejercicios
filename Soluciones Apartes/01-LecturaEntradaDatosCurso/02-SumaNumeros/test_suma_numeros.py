...
class TestSumNumbers(unittest.TestCase):
    ...
    def test_valid_input(self, mock_input):
        ...
        self.assertIn('Resultado De La Suma => ', output, 'OJO! MENSAJES IGUALES A LOS DE INSTRUCCIONES O SUGERENCIAS.')

    # *** QUINTO ESCENARIO => Confirmar Los Mensajes De Los Prints() ***
    @patch('builtins.input', side_effect=['33', '12'])
    def test_valid_messages_outputs(self, mock_input):
        ...
        pattern_three = re.compile(r'Resultado\s+De\s+La\s+Suma => \s*([\d.]+)', re.IGNORECASE)
        ...
        self.assertTrue(three, "❌ No se encontró 'Resultado De La Suma => ' en la salida")

        # Extraer Valores
        number_one = float(one.group(1))
        number_two = float(two.group(1))
        result_sum = float(three.group(1))

        # Verificaciones Y Afirmaciones
        self.assertAlmostEqual(number_one, 33.0, places = 2, msg = "❌ Número Uno Incorrecto (Debería Ser value_one = 33)")
        self.assertAlmostEqual(number_two, 12.0, places = 2, msg = "❌ Número Dos Incorrecto (Debería Ser value_two = 12)")
        self.assertAlmostEqual(result_sum, 45.0, places = 2, msg = "❌ Resultado Suma Incorrecto (Debería Ser sum = value_one + value_two)")
