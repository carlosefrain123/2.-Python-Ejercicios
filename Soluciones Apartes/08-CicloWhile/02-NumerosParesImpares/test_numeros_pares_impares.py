...
# ** ======== FUNCIONA EN UDEMY POR QUE SE GUARDA EN DISCO ========
    ...
    def test_structure_try(self):
        source_code = inspect.getsource(numeros_pares_impares)
    ...
    def test_print_message(self, mock_input):
        ...
        self.assertEqual(
            prompt_one,
            '*** Sistema De Procesamiento De Números Pares E Impares ***',
    ...
    def test_input_messages(self, mock_input):
        ...
        self.assertEqual(
            prompt_one,
            'Ingrese La Cantidad De Números Enteros Que Vas A Contabilizar: ',
            'El Primer Mensaje No Coincide Con El Esperado.'
        )
    ...
    # *** NOVENO ESCENARIO => Validar Conteo de Pares / Impares ***
    @patch('builtins.input', side_effect = ['3', '2', '4', '6'])
    def test_conteo_pares(self, mock_input):
        reload(numeros_pares_impares)
        
        output = self.stdout_capture.getvalue()
        
        self.assertIn('La Cantidad De Números Pares Son: 3', output, '❌ Conteo de pares incorrecto')
        self.assertIn('La Suma De Los Números Pares Es: 12', output, '❌ Suma de pares incorrecta')

    # *** DECIMO ESCENARIO => Validar Promedio Impares ***
    @patch('builtins.input', side_effect = ['2', '3', '5'])
    def test_promedio_impares(self, mock_input):
        reload(numeros_pares_impares)

        output = self.stdout_capture.getvalue()
        
        self.assertIn('El Promedio De Los Números Impares Es: 4.0', output, '❌ Cálculo promedio incorrecto')

    # *** DECIMOPRIMER ESCENARIO => Validar Cero Impares ***
    @patch('builtins.input', side_effect = ['2', '4', '6'])
    def test_cero_impares(self, mock_input):
        reload(numeros_pares_impares)
        
        output = self.stdout_capture.getvalue()
        
        self.assertIn('El Promedio De Los Números Impares Es: 0.0', output, '❌ Manejo de cero impares fallido')

    # *** DECIMOSEGUNDO ESCENARIO => Validar Formato Salida ***
    ...
    def test_formato_salida(self, mock_input):
        ...
        self.assertRegex(output, r'Pares Son: \d+\n.*Suma.*\n.*Promedio', '❌ Formato de salida incorrecto')
